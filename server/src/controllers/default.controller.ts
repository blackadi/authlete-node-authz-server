...
export const defaultController = {
  ...
  handleDefault: async (req: Request, res: Response) => {
    ...
    res.render("index", {
      code,
      title: code !== 'No code provided' ? "Authorization response content(including authorization code)" : "No authorization code received. Start the authorization process by clicking the button below",
      state,
      iss
    });
  },
