...
} from "../../node_modules/@authlete/typescript-sdk/dist/commonjs/models/operations";
...
export class DiscoveryService {
  async getConfiguration(
    req: Request
  ): Promise<ServiceConfigurationApiResponse> {
    const { ...reqBody }: ServiceConfigurationApiRequest = req.body;
    reqBody.serviceId = serviceId;
    reqBody.pretty = true;

    req.logger("Discovery parameters", { reqBody }) ||
      logger("Discovery parameters", { reqBody });
    ...
    const fetchreq = await fetch(
      `${process.env.AUTHLETE_BASE_URL}/api/${process.env.AUTHLETE_SERVICE_ID}/service/configuration?pretty=true`,
      {
        method: "GET",
        headers: {
    ...
    if (!fetchreq.ok) {
      logger.error("Failed to fetch service configuration", {
        ...
        statusText: fetchreq.statusText,
      });
      throw new Error(
        `Failed to fetch service configuration: ${fetchreq.statusText}`
      );
    }
